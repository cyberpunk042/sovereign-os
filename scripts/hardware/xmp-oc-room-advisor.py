...
DEFAULTS = {
    ...
    # GPU sustained draw under load (SDD-993). gpu1 = the always-on PRIMARY
    # (RTX PRO 6000 Blackwell Max-Q, 300 W — NOT the 600 W workstation card);
    # gpu2 = the SECONDARY that powers down in idle (RTX 5090, ~350 W). The
    # RTX 4090 OcuLink eGPU is a separate host-resident card, not modeled here.
    "gpu1_sustained_w": 300,                # RTX PRO 6000 Max-Q (primary)
    "gpu2_sustained_w": 350,                # RTX 5090 (internal secondary)
...
WORKLOAD_MODE_TO_RUNTIME_KNOBS: dict[str, dict[str, Any]] = {
    "idle": {
        ...
        "gpu_oc_notch": 0,
        "dual_gpu_active": False,   # idle = PRO 6000 only; RTX 5090 powered down
...
def render_status_human(cfg: dict, load: dict, verdict: dict) -> str:
    ...
    lines.append(f"    GPU1 (RTX PRO 6000): {load['gpu1_w']:.0f}W")
    lines.append(f"    GPU2 (RTX 5090):     {load['gpu2_w']:.0f}W")
