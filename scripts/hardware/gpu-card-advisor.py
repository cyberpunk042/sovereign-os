...
KNOWN_CARDS: dict[str, dict[str, Any]] = {
    ...
    "RTX PRO 6000": {
        ...
        "vram_type": "GDDR7",
        "stock_tdp_watts": 600,
        "operator_recommended_cap_watts": [500, 600],
        "pcie_rated": "PCIe 5.0 x16",
        "advisories": [
            "RTX PRO 6000 Blackwell — first card with 96 GB usable VRAM at "
            ...
            "returning (10, 0). Quantization tools that use FP8/FP4 (e.g. "
...
def cmd_dual_card(args: argparse.Namespace) -> int:
    ...
    if is_sain01_dual:
        findings.append(
            "SAIN-01 dual-card layout detected. Cross-reference R270 pcie-policy "
            "for x8/x8 split confirmation + R252 power-status budget for "
            "350+600=950 W sustained vs 1600 W PSU rated (with derating "
            "→ 1360 W budget → 410 W headroom)."
        )
        findings.append(
