#!/usr/bin/env bash
# scripts/build/05-substrate-prepare.sh — substrate-adapter prep.
#
# Reads the active profile + active substrate (per Q-001 Gate 2);
# emits substrate-native config (mkosi.conf / live-build config/ /
# treefile.yaml / configuration.nix). Working hypothesis substrate:
# mkosi on Debian 13 (per SDD-003 primary recommendation). Operator
# overrides via SOVEREIGN_OS_SUBSTRATE env var until Gate 2 locks.

__SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=./lib/common.sh
. "${__SCRIPT_DIR}/lib/common.sh"
# shellcheck source=./lib/observability.sh
. "${__SCRIPT_DIR}/lib/observability.sh"
# shellcheck source=./lib/distro.sh
. "${__SCRIPT_DIR}/lib/distro.sh"

STEP_ID="05-substrate-prepare"

: "${SOVEREIGN_OS_PROFILE:=sain-01}"
: "${SOVEREIGN_OS_SUBSTRATE:=mkosi}"
load_profile "${SOVEREIGN_OS_PROFILE}"

emit_substrate_metric() {
  emit_metric sovereign_os_build_step_substrate_total 1 \
    "substrate=\"${SOVEREIGN_OS_SUBSTRATE}\",profile=\"${SOVEREIGN_OS_PROFILE}\",result=\"$1\""
}

# The emitted config is a function of the ADAPTER CODE and the env knobs
# below, not just this step + the profile: a fixed emitter (or a changed
# snapshot pin / signing key / root-password posture) silently reused the
# stale build/<profile>/mkosi.conf because none of them were hashed —
# the 2026-06-12 Run-console build failed on main-only apt AFTER the
# repos fix landed, because step 05 skipped itself.
adapter_for_hash="${__SCRIPT_DIR}/adapters/${SOVEREIGN_OS_SUBSTRATE}-emit.sh"
# Signature of the repo working tree that gets BAKED into the image (mkosi-emit
# rsyncs it into mkosi.extra → /opt/sovereign-os). WITHOUT this, editing a baked
# file (first-boot hooks, systemd units, scripts, configs) changes no hashed
# input, so 05 + 07 skip and the 'rebuild' silently reuses the stale image
# ('rebuild did nothing'). Git-based (HEAD + uncommitted tracked diff +
# staged/untracked list) — fast, and naturally ignores gitignored build/ +
# target/, matching the bake excludes. 'nogit' when not a git checkout (a
# tarball build), where the operator falls back to `orchestrate.sh rewind`.
# safe.directory: the build runs as root (sudo) but the repo is operator-owned,
# so plain git aborts with 'dubious ownership' → sig would silently be 'nogit'.
_gitr() { git -c safe.directory="${SOVEREIGN_OS_ROOT}" -C "${SOVEREIGN_OS_ROOT}" "$@" 2>/dev/null; }
repo_sig="nogit"
if command -v git >/dev/null 2>&1 && _gitr rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  repo_sig="$( { _gitr rev-parse HEAD; _gitr status --porcelain; _gitr diff HEAD; } \
               | sha256sum | cut -d' ' -f1)"
fi
# Resolve the signing key the SAME way mkosi-emit does (env override, else the
# SDD-015 default /etc/sovereign-os/keys) so a panel build (env set) and a CLI
# build (auto-discovered) produce the SAME hash instead of spuriously rebuilding.
sb_key_hash="${SOVEREIGN_OS_PK_KEY:-${SOVEREIGN_OS_MOK_KEY:-}}"
[ -z "${sb_key_hash}" ] && [ -f /etc/sovereign-os/keys/mok.key ] && sb_key_hash="/etc/sovereign-os/keys/mok.key"
inputs_hash="$(state_inputs_hash "${BASH_SOURCE[0]}" "${SOVEREIGN_OS_PROFILE_FILE}" \
  "${adapter_for_hash}" \
  "snapshot=${DEBIAN_SNAPSHOT:-}" \
  "epoch=${SOURCE_DATE_EPOCH:-}" \
  "sb_key=${sb_key_hash}" \
  "root_pw=$([ -n "${SOVEREIGN_OS_ROOT_PASSWORD:-}" ] && echo set || echo unset)" \
  "bake_dev=${SOVEREIGN_OS_BAKE_DEV_TOOLS:-}" \
  "bake_selfdef=${SOVEREIGN_OS_BAKE_SELFDEF:-}" \
  "node_major=${SOVEREIGN_OS_NODE_MAJOR:-22}" \
  "artifact=${SOVEREIGN_OS_ARTIFACT:-image}" \
  "distro=${SOVEREIGN_OS_DISTRO:-debian}" \
  "suite=$(distro_suite)" \
  "repo_sig=${repo_sig}")"

if ! state_step_should_run "${STEP_ID}" "${inputs_hash}"; then
  log_info "step ${STEP_ID} already completed with matching inputs — skipping"
  exit 0
fi

log_step_header "${STEP_ID}" "substrate-adapter prep (substrate=${SOVEREIGN_OS_SUBSTRATE})"
state_step_start "${STEP_ID}" "${inputs_hash}"

: "${SOVEREIGN_OS_BUILD_OUT:=${SOVEREIGN_OS_ROOT}/build/${SOVEREIGN_OS_PROFILE}}"
mkdir -p "${SOVEREIGN_OS_BUILD_OUT}"

case "${SOVEREIGN_OS_SUBSTRATE}" in
  mkosi)
    adapter="${__SCRIPT_DIR}/adapters/mkosi-emit.sh"
    ;;
  live-build)
    adapter="${__SCRIPT_DIR}/adapters/live-build-emit.sh"
    ;;
  installer-cdd|ubuntu-autoinstall)
    # The STANDARD installer ISO for the selected distro. Unlike mkosi/live-build
    # there is no config tree to emit: the substrate's own build.sh owns the whole
    # thing (profile, local package repo, answer file, the CD mirror) and runs in
    # step 07. This step only needs to confirm the builder exists and let the
    # pipeline through — adding a substrate to step 07 alone left step 05
    # rejecting it as unknown (2026-07-26, my own miss).
    #
    # Two builders, because the install mechanisms are incompatible:
    #   installer-cdd      simple-cdd -> debian-cd -> debian-installer + preseed
    #   ubuntu-autoinstall official Ubuntu ISO -> Subiquity + autoinstall YAML
    # (Ubuntu dropped debian-installer at 20.04; preseed does not drive Subiquity.)
    case "${SOVEREIGN_OS_SUBSTRATE}" in
      ubuntu-autoinstall) _builder_dir="ubuntu-autoinstall" ;;
      *)                  _builder_dir="installer-cdd" ;;
    esac
    _cdd="${SOVEREIGN_OS_ROOT}/scripts/build/${_builder_dir}/build.sh"
    if [ ! -x "${_cdd}" ]; then
      log_error "${SOVEREIGN_OS_SUBSTRATE} substrate selected but ${_cdd} is missing or not executable"
      emit_substrate_metric fail
      state_step_fail "${STEP_ID}" "${SOVEREIGN_OS_SUBSTRATE}-builder-missing"
      exit 1
    fi
    log_info "substrate ${SOVEREIGN_OS_SUBSTRATE} — the installer ISO is built by ${_cdd} in step 07"
    log_info "  distro: $(distro_label)  suite: $(distro_suite)"
    log_info "  (no config tree to emit here; the builder owns its own profile + mirror)"
    # WRITE THE ENV HANDOFF BEFORE EXITING. Step 07 SOURCES this file and it
    # `export`s SOVEREIGN_OS_SUBSTRATE, overriding whatever the orchestrator
    # set. Exiting early here left the PREVIOUS run's value in place, so step 05
    # ran installer-cdd while step 07 ran live-build, hit "Skipping binary_iso,
    # already done", left a 1-hour-old ISO untouched and exited 0 — the operator
    # flashed the stale image again (2026-07-26, my own bug).
    env_file="${SOVEREIGN_OS_STATE_DIR}/env-substrate.sh"
    # DISTRO/SUITE ride along for the same reason SUBSTRATE does: step 07 sources
    # this file and its exports win over the orchestrator's. A handoff that
    # carried only the substrate would let a stale trixie leak into an Ubuntu
    # build — the 2026-07-26 failure with a different variable.
    cat > "${env_file}" <<EOF
# auto-generated by ${STEP_ID}
export SOVEREIGN_OS_SUBSTRATE="${SOVEREIGN_OS_SUBSTRATE}"
export SOVEREIGN_OS_BUILD_OUT="${SOVEREIGN_OS_BUILD_OUT}"
export SOVEREIGN_OS_DISTRO="${SOVEREIGN_OS_DISTRO}"
export SOVEREIGN_OS_SUITE="$(distro_suite)"
EOF
    log_info "env handoff: ${env_file} (substrate=${SOVEREIGN_OS_SUBSTRATE} distro=${SOVEREIGN_OS_DISTRO} suite=$(distro_suite))"
    emit_substrate_metric success
    state_step_complete "${STEP_ID}"
    exit 0
    ;;
  rpm-ostree|nixos)
    log_error "substrate '${SOVEREIGN_OS_SUBSTRATE}' is an ALT path (B/C); adapter not implemented in foundation phase"
    emit_substrate_metric not-implemented
    state_step_fail "${STEP_ID}" "alt-substrate-not-implemented"
    exit 1
    ;;
  *)
    log_error "unknown substrate: ${SOVEREIGN_OS_SUBSTRATE} (valid: mkosi, live-build, installer-cdd, ubuntu-autoinstall, rpm-ostree, nixos)"
    emit_substrate_metric unknown
    state_step_fail "${STEP_ID}" "unknown-substrate"
    exit 1
    ;;
esac

if [ ! -x "${adapter}" ]; then
  log_error "${SOVEREIGN_OS_SUBSTRATE} adapter missing or not executable: ${adapter}"
  emit_substrate_metric missing-adapter
  state_step_fail "${STEP_ID}" "missing-${SOVEREIGN_OS_SUBSTRATE}-adapter"
  exit 1
fi

if [ -n "${SOVEREIGN_OS_DRY_RUN:-}" ]; then
  log_info "DRY-RUN — would invoke ${adapter} ${SOVEREIGN_OS_PROFILE_FILE} ${SOVEREIGN_OS_BUILD_OUT}"
  emit_substrate_metric skip
  # Record 'dry-run', NOT 'completed' — completing here with the real
  # inputs_hash makes the next REAL run skip this step body entirely.
  state_step_dry_run "${STEP_ID}"
  exit 0
fi

if "${adapter}" "${SOVEREIGN_OS_PROFILE_FILE}" "${SOVEREIGN_OS_BUILD_OUT}"; then
  log_info "${SOVEREIGN_OS_SUBSTRATE} adapter emit succeeded → ${SOVEREIGN_OS_BUILD_OUT}"
  emit_substrate_metric success
else
  rc=$?
  log_error "${SOVEREIGN_OS_SUBSTRATE} adapter emit failed (rc=${rc})"
  emit_substrate_metric fail
  state_step_fail "${STEP_ID}" "adapter-emit-failed-${rc}"
  exit 1
fi

env_file="${SOVEREIGN_OS_STATE_DIR}/env-substrate.sh"
cat > "${env_file}" <<EOF
# auto-generated by ${STEP_ID}
export SOVEREIGN_OS_SUBSTRATE="${SOVEREIGN_OS_SUBSTRATE}"
export SOVEREIGN_OS_BUILD_OUT="${SOVEREIGN_OS_BUILD_OUT}"
EOF
log_info "env handoff: ${env_file}"

state_step_complete "${STEP_ID}"
log_info "step ${STEP_ID} complete"
