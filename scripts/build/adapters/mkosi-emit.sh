...
# NOTE: this body is a PLAIN dedent, not an f-string — it is full of shell
# ${...} that f-string interpolation would mangle. The one build-time value
# it needs is substituted below via a sentinel.
...
postinst.write_text(postinst.read_text().replace(
    "__SOVEREIGN_OS_DISTRO__", distro))
postinst.chmod(0o755)

# ---- frontend policy → systemd preset (mkosi.extra overlay) ----------------
# mkosi runs `systemctl preset-all`, which enables EVERY unit in
# /etc/systemd/system carrying an [Install] section. install-gui-dashboards.sh
# deliberately DISABLES sovereign-frontend-kiosk.service when the chosen
# frontend is a desktop — and preset-all then turned it straight back on.
#
# The result on the operator's box (2026-07-30): the appliance booted
# graphical.target with BOTH sddm and a fullscreen kiosk browser wanting the
# display, which is a very good fit for "a weird login page that was broken and
# then froze". The ISO-installed system is clean because that path never runs
# preset-all — so the two installers disagreed for a reason nobody could see.
#
# This repo already hit the same trap with the selfdef fleet and dodged it by
# keeping those units OUT of /etc/systemd/system. The kiosk unit has to live
# there (the runtime frontend switch enables it), so state the policy
# DECLARATIVELY instead: a preset is exactly the mechanism preset-all obeys.
_fe_kiosk = frontend_default in ("dashboards-kiosk", "open-computer-kiosk")
_preset_dir = out_dir / "mkosi.extra" / "usr" / "lib" / "systemd" / "system-preset"
_preset_dir.mkdir(parents=True, exist_ok=True)
(_preset_dir / "80-sovereign-frontend.preset").write_text(
    "# Generated by mkosi-emit.sh from the profile's provisioning.frontend.\n"
    "# preset-all obeys this, so the chosen frontend survives it.\n"
    f"# frontend = {frontend_default}\n"
    + ("enable sovereign-frontend-kiosk.service\n" if _fe_kiosk
       else "disable sovereign-frontend-kiosk.service\n")
    + ("disable sddm.service\ndisable gdm3.service\n" if _fe_kiosk
       else "")
)
print(f"mkosi-emit: frontend preset written (frontend={frontend_default}, "
      f"kiosk={'enabled' if _fe_kiosk else 'disabled'})", file=sys.stderr)
