#!/usr/bin/env python3
"""scripts/cockpit/mfa-grant-revocations-queue.py — SDD-069 MS5b.

Fifth (and final) in the IPS-pentet cockpit family. Completes the
pentet-paired-handle row.

Reads /var/lib/selfdef/mfa-grant-revocations/pending-restores.json.
Path overridable via SOVEREIGN_OS_MFA_GRANT_REVOCATIONS_PENDING_PATH.
"""
...
DEFAULT_PATH = Path(
    os.environ.get(
        "SOVEREIGN_OS_MFA_GRANT_REVOCATIONS_PENDING_PATH",
...
def render_restore_command(entry: dict[str, Any]) -> str:
    ...
    return f"selfdefctl restore-mfa-grant '{safe_handle}'"


def render_scope(entry: dict[str, Any]) -> str:
    scope = entry.get("grant_scope", "All")
    if isinstance(scope, str):
        return scope
    if isinstance(scope, dict):
        if "Specific" in scope:
            items = scope["Specific"]
...
def render_human(queue: list[dict[str, Any]]) -> str:
    ...
    lines = [
        "SDD-069 — pending operator-MFA-grant-restore queue",
        f"{'─' * 14} {'─' * 6} {'─' * 16} {'─' * 32}",
        f"{'principal':<14} {'left':<6} {'scope':<16} {'reason':<32}",
        f"{'─' * 14} {'─' * 6} {'─' * 16} {'─' * 32}",
    ]
    for entry in queue:
        ...
        lines.append(f"{principal:<14} {left:<6} {scope:<16} {reason:<32}")
