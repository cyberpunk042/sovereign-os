...
|---|---:|---|
| workspace crates | 729 | `crates/*/` |
| dashboards (d-nn) | 29 | `webapp/d-*/` |
| cockpit panels (total) | 64 | `webapp/*/index.html` |
| sdd files | 229 | `docs/sdd/<NNN>-*.md` |
...
- **Cockpit model-quantization + actionability arc (2026-07-19)** — an operator-driven cockpit UX pass: pluggable jobs-registry backend (JSON default / opt-in SQLite + bidirectional migration via `SOVEREIGN_OS_JOBS_STORE`, #239); one-click enum conversion of the jobs-store backend toggle + 13 sibling mode/profile controls, plus a dead-label sweep against the button values across the 61 panels (#239/#241); a load-time **quantization picker** on D-03 + D-21 (shared `webapp/_shared/quant-picker.js`, backed by a group-by-base catalog view `GET /api/models-catalog/by-base`) and a read-only quant-variants section on D-23 (#243/#246); **quant/dtype-aware `models warm`** (SDD-049 Stage 4 — reports the loaded precision + active runtime mode and flags a drift when a tier serves a different model than `model-state.json` records, #250); and the control-surface now lifts a tool-level `warning` into the result line so that drift reaches the operator instead of hiding behind a bare `exit 0` (#253). All client-side cockpit changes preserve the single-file lockstep + R10212 read-only boundary.
...
1. ~~**F-2026-070** — Merge the `network-edge` / `edge-firewall` / `d-12-networking` triplet at the API level~~ **DONE (2026-07-20)** — unified `scripts/operator/networking-api.py` daemon (port 8139) serves all backward-compatible root paths (`/detect`, `/state`, `/candidates`, `/recommend`, `/install-plan`, `/api/d-12/snapshot`, `/api/d-12/stream`) plus namespaced variants (`/network-edge/*`, `/edge-firewall/*`); systemd unit `sovereign-networking-api.service`; dashboard-catalog + dashboard-routes + panel-api-routes regenerated; lint exceptions added for intentional triplet port-sharing; 23-test contract coverage in `test_networking_api_contract.py`. **Remaining (operator call):** panel fetch-url migration to namespaced paths (if desired) + old daemon deprecation.
...
**sovereign-os Rust workspace: 723 crates total** (counts-as-contract enforced by `tests/lint/test_context_md_counts.py`)
...
repo has **723 crates** (was 475 when this caveat was written 2026-05-27 —
