...
(function () {
  ...
  function mount() {
    ...
    var BASES = [];
    // Human context window: 1000000 → "1M", 131072 → "131K", 4096 → "4K".
    function fmtCtx(t) { t = +t || 0; return t >= 1e6 ? +(t / 1e6).toFixed(1) + 'M'
      : t >= 1e3 ? Math.round(t / 1e3) + 'K' : (t || '?'); }
    function baseCtx(b) { return (b.variants && b.variants[0] && b.variants[0].context_window_tokens)
      || b.context_window_tokens || 0; }
    function fillSelect(bases) {
      ...
      function opt(b) { var c = baseCtx(b); return '<option value="' + esc(b.base) + '">' + esc(b.base)
        + (c ? ' · ' + fmtCtx(c) + ' ctx' : '')
    ...
    function renderVariants(base) {
      ...
      varsEl.innerHTML = g.variants.map(function (v) {
        ...
        var ct = +v.context_window_tokens || 0;
        var ctx = ct ? '<span class="qp-ctx' + (ct >= 1e6 ? ' big' : '') + '">' + fmtCtx(ct) + ' ctx</span>' : '';
        var prm = v.params_b != null ? v.params_b + 'B' : '';
        var rest = [vram, prm, esc(v.status || '')].filter(Boolean).join(' · ');
        return '<button type="button" class="qp-variant" aria-pressed="false" data-id="' + esc(v.id)
          ...
          + '</span><span class="qp-meta">' + ctx + (ctx ? ' · ' : '') + rest + '</span></button>';
      }).join('');
    }
    function choose(btn) {
