...
| Q-047-B | selfdef-owned controls (selfdef/perimeter/D-12..D-18) | Stay a signed proxy to selfdef (recommended, preserves R10212 producer/consumer) vs execute locally | **answered (D-024, 2026-07-15)** — ratified **stay signed-proxy** (SDD-985 D2); enforced by `_action_exec.py:53,260-270` (409) + `control-surface.js:36` `PROXY_ONLY` + `test_control_surface_execute_boundary.py`. Preserves R10212 producer/consumer. |
