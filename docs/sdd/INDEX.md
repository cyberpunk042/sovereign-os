...
| 024 | role-server hardening posture | review | post-Gate-5 | Round 96-99 codification: 4 IaC drop-ins (auditd / fail2ban / unattended-upgrades / sshd) deployed by apply-server-hardening hook on role-server profiles; load-bearing invariants pinned at Layer 1 lint (silent weakening fails CI); operator override path via lexicographically-later drop-ins; `sshd -t` validation gate prevents lock-out. Closes the "hardened packages with stock configs" gap. Q24-A..Q24-D open. |
