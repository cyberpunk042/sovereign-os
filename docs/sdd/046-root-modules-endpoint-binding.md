# SDD-046 — root-modules endpoint binding (proxy mode disabled)

> **Renamed 2026-07-19**: the upstream project `root-ghostproxy` is now `root-modules` — operator verbatim: *"root-ghostproxy has just been renamed into root-modules. lets update the repo as such. its at first and by default a root or home folder upgrader, evolver and secondly you can install supplementary modules like the ghostproxy combo."* "Ghostproxy" now names the proxy module combo this binding keeps OFF. This SDD, its hooks, profile wiring, and lint gate were renamed in lockstep; legacy env names (`SOVEREIGN_OS_ROOT_GHOSTPROXY_DIR`) and pre-rename `~/root-ghostproxy` checkouts stay honored. Plain-"ghostproxy" wire identifiers (metric families `sovereign_os_ghostproxy_endpoint_*`, unit `sovereign-ghostproxy-verify.*`, `*_GHOSTPROXY_*` gate envs, profile legacy key) are KEPT — flagged as a possible future migration, not renamed here.

> Status: draft
> Owner: operator-supervised; agent-authored
> Last updated: 2026-07-03
> Closes findings: none (activates the fourth repo's consumption surface)
> Derived from: operator directive 2026-07-03 (verbatim, sacrosanct): *"Lets prepare root-ghostproxy for sovereign-os usage, we will use use the repo without the proxy mode enabled."*; root-modules PR #3 (merged 2026-07-03) `docs/sovereign-os-endpoint-usage.md`; SDD-001 (boundary contract); SDD-038 (cross-repo binding doctrine)

## Mission

`cyberpunk042/root-modules` — the ecosystem's system-AI-safety-setup
...
proxy/IPS half of root-modules (transparent L2 bridge, management
...
SDD-001 explicitly scoped root-modules re-activation OUT of the
boundary contract ("Does NOT decide how root-modules re-activates").
...
- The install step has no lifecycle placement — an operator would run
  root-modules's installer ad-hoc, outside the profile's declared
...
- Nothing distinguishes the sanctioned posture (endpoint mode, proxy
  OFF) from an accidental full install: root-modules's own
...
A1. The binding MUST consume root-modules through its own install
...
G2. Drift between deployed endpoint-safety state and root-modules's
...
G4. Status rows across the repo's docs say the truth: root-modules
...
- Does NOT replace selfdef. Composition per root-modules's usage
  doc: root-modules governs the AI-agent tool-call surface;
  selfdef governs the OS runtime-defense surface (Tetragon perimeter,
  notifiers, escalations). Both run on the node.
- Does NOT bake root-modules into the OS image at build time —
...
- Does NOT author a typed-TOML mirror (SDD-038 shape) yet — the
  ...
  root-modules state should feed compliance rollups.
...
| Q-046-002 | Should the root-modules checkout + endpoint install be baked into the image build (mkosi skeleton/postinst) instead of first-boot? Trade-off: reproducible image vs $HOME-scoped install semantics. | open |
| Q-046-003 | Should root-modules publish a typed manifest (SDD-038 shape) so `sovereign-osctl compliance status` gains a ninth instrument axis (AI-agent-envelope state)? | open |
| Q-046-004 | Which user's `$HOME` receives the envelope on SAIN-01 — root, the operator user, or both? root-modules is type=root by scope (not path); the hook currently installs for the invoking user's `$HOME`. | open |
...
| Artifact | Purpose |
|---|---|
| `scripts/hooks/post-install/root-modules-endpoint-install.sh` | First-boot install hook. Triple-gate: report-only dry-run by default; `SOVEREIGN_OS_CONFIRM_GHOSTPROXY_INSTALL=YES` applies; `SOVEREIGN_OS_DRY_RUN=1` forces report-only. Mode pinned `endpoint`; profile default `base` (env-overridable); checkout dir `SOVEREIGN_OS_ROOT_GHOSTPROXY_DIR` (default `${HOME}/root-modules`). |
| `scripts/hooks/recurrent/root-modules-verify.sh` | Weekly read-only drift verify: `install.sh --check --profile base --mode endpoint`. OBSERVATION, not REMEDIATION (same contract as `audit drift`, D-018). |
...
### Upstream contract (root-modules side, already merged)

root-modules PR #3 (merged 2026-07-03) ships the consumption guide
...
- Operator directive verbatim + reading: `cyberpunk042/root-modules
  wiki/log/2026-07-03-sovereign-os-endpoint-prep-directive.md`
- Upstream consumption guide: `cyberpunk042/root-modules
