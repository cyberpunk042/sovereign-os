...
Running tally: **18 bugs caught** as of Round 144. Each surfaced + fixed
...
| 18 | `sovereign-osctl hooks add` (R141) DESTROYED the profile YAML's header comments — the python heredoc had `with open(pfile, "w") as f: pass` to "create the file" BEFORE the subsequent `with open(pfile) as f_orig: orig_lines = f_orig.readlines()` block that was meant to capture the header. The `open(..., "w")` mode TRUNCATES — so by the time the header-capture read ran, the file was empty. Net effect: every `hooks add` blew away the `# yaml-language-server: ...` hint + the multi-line description block. Caught during R144 git status diff after R141 + R143 smoke testing showed `profiles/minimal.yaml` as modified despite the test trap restoring it (the smoke tests outside the test ran the buggy path) | removed the spurious `open(pfile, "w") as f: pass` line. Read the original FIRST, capture header lines, THEN open for write. The L3 test (`test_hooks.sh`) was technically passing because it backed up + restored the entire file; the bug was visible only in real operator use. Added explicit comment marking the fix at the source. Adds a new Learning if it recurs | `test_hooks.sh` (L3, indirect — bug was outside its restore boundary) |
