...
| Q-002 | Profile inheritance model (single-parent · composition) | PR 5 → Gate 3 | **resolved (SDD-004, 2026-05-16)** — single-parent inheritance + composition via mixins fully implemented; 5 profiles + 6 mixins shipped; schema validates raw + resolved at L1; merge rules pinned in SDD-004 §; Q5-A..Q5-E sub-questions tracked in SDD-004 for future enhancement. Operator-driven fork/overlay workflows are operator-side (no sovereign-os code changes required) — `sovereign-osctl profiles {show,show-effective,switch}` covers in-place profile lifecycle. |
