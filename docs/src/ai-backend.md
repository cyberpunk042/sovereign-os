...
| `POST /v1/models/default` | designate what the `"auto"` alias — and an omitted or empty `model` — resolves to. Separate from `background` on purpose: `background` moves deliberation off the primary, and reusing it here would drag foreground traffic along whenever an operator pointed deliberation at something cheap. A designated-but-unloaded id resolves to `null`, so routing falls back to the primary honestly rather than stranding on a tier that went away | `{id}` → `{default:id, active:id\|null}` |
| `POST /v1/embeddings` | **OpenAI embeddings shim** — relayed to the Router-tier embedder at `SOVEREIGN_GATEWAY_EMBED_ENDPOINT`. The `model` field is rewritten to `SOVEREIGN_GATEWAY_EMBED_MODEL` rather than forwarded, since the upstream serves exactly one model and has never heard of gateway-side aliases like `auto`. Deliberately **not** a proxy-registry backend: `auto` falls back to the last-registered chat tier, and a pooling model there could be handed a chat completion it cannot answer. `503` when no embedder is configured | `{model?, input}` → `{object:"list", data:[{embedding:[…], index}], usage}` |
...
| `POST /v1/corpus/search` | show which passages the RAG **would** ground a prompt on, without generating. Runs the same recall → rerank → select path the grounding uses, so it reports what a prompt actually receives rather than a parallel implementation. Ids are `{file}#{chunk}`, so an answer can be traced to its source. `stage` restricts the pipeline for measurement — `fused` (default, lexical + dense), `hybrid` (lexical only, the pre-dense baseline), `dense` (embedding ranking with no rerank); a change to any one stage moves the final number without saying which, and `dense` is what tells you whether the embedder found the passage at all. `503` when no corpus is loaded — distinct from an empty result, since "RAG is off" and "nothing matched" are different answers | `{query, k?, stage?}` → `{query, k, corpus_docs:N, hits:[{rank, id, text}]}` |
...
| `POST /v1/data-plane/token-law/fuse` | **M00155 token-law fusion** (SDD-507, F00792/F00797) — fuse the NAMED laws (grammar/regex/denylist/negated-regex/policy) at a generated prefix into one vocab allow-mask, DERIVING each layer's bitset from a real source (vs `allowed-mask`'s pre-packed bitsets). Checkpoint-free: the mask depends only on the sources + supplied `vocab`, never a loaded model | `{schema?, regex?, denylist?:[…], regex_denylist?:[…], policy_planes?:[[u64,…],…], generated?, vocab:[…]}` → `{kind:"token-law-fuse", mask[], allowed_tokens, per_layer[], layers_active[], stop}` |
