...
| `POST /v1/corpus/search` | show which passages the RAG **would** ground a prompt on, without generating. Runs the same wide-recall → coverage-rerank → backfill path the grounding uses, so it reports what a prompt actually receives rather than a parallel implementation. Ids are `{file}#{chunk}`, so an answer can be traced to its source. `503` when no corpus is loaded — distinct from an empty result, since "RAG is off" and "nothing matched" are different answers | `{query, k?}` → `{query, k, corpus_docs:N, hits:[{rank, id, text}]}` |
