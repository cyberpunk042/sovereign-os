...
| `POST /v1/branch-scheduler/tick-v2` | **M007 tick v2** — the richer tick that *consumes* the M008 building blocks: memory recall (bloom), the branch predictor (M00121), the two-level rule table (M00119) for Verify, and microcode (M00113) for Commit | `{batch, rule_table:[[u8,…],…], event_class:[usize;8], memory_bank:[u64,…], verify_min_score?}` → `{kind:"branch-scheduler-tick-v2", result:{base, recall[], predicted_commit, rule_verified, predictor_accuracy}}` |
