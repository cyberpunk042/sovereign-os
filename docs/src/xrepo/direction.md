...
sovereign-os  ──CONSUMES FROM──▶  root-modules  (AI-agent safety envelope, endpoint mode — SDD-046)
selfdef       ──CONSUMES FROM──▶  info-hub + sovereign-os
info-hub      ──OBSERVES──────▶  sovereign-os + selfdef
root-modules  (active — endpoint-mode dependency; proxy half disabled per operator directive 2026-07-03)
```
...
| `cyberpunk042/root-modules` | AI-agent tool-call safety policy (machine-level Claude Code + opencode envelope, integrity sentinel) — consumed on SAIN-01 in endpoint mode via its own `install.sh` (SDD-046); proxy/IPS half disabled | OS construction; runtime OS defense (selfdef) |
