...
- `sovereign_os_operator_auth_tier_api_request_total{endpoint,result}` — R501 (E11.M7++) read-only REST API request counter. Endpoint ∈ {version, tiers, registry, show, matrix, healthz, root, unknown, post, put, delete, patch}; result ∈ {ok, 400, 404, 405, 500}. Mutation verb `set` returns 405 (operator §17 sovereignty boundary — tier mutation stays CLI-only). Daemon: `scripts/operator/auth-tier-api.py`; systemd unit: `sovereign-auth-tier-api.service` (loopback-bind default, port 8091).
