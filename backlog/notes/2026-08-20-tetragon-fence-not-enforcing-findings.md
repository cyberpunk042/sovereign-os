...
The container scope's `matchNamespaces: Mnt NotIn host_ns` was replaced. Two mechanisms were empirically disproven in monitor mode: matchNamespaces (matched every sandboxed host service — systemd/NetworkManager/python), and `matchParentBinaries(conmon,crun) followChildren` (matched podman's host-side network cascade — netavark + triggered nm-dispatchers + host python3.14). The precise discriminator is the **cgroup**: podman gives a container's workload a leaf cgroup literally named `container` (`.../libpod-<id>.scope/container`); host services carry their `*.service` name. A `matchData` block resolving `cgroups.dfl_cgrp.kn.name` + `Equal "container"` matched **only** the container's execs (`/bin/sh`, crun's `/dev/fd/3`) and **zero** host binaries in monitor validation. Implemented in `tetragon-policy-load.sh` (container scope) + knobs lint + `test_container_scope_uses_cgroup_matchdata`. Needs no extra daemon flags (unlike matchParentBinaries, which needs `--parents-map-enabled`). **VALIDATED end-to-end (2026-08-20):** armed in container scope, `podman run busybox sh -c 'id && echo ID-RAN'` → container **Killed**, `NENFORCE=5`, ID-RAN never printed; the desktop that launched it was untouched. Sigkill-alone was sufficient to kill the workload (Tetragon docs note Signal+Override for *guaranteed* operation-prevention — a belt-and-suspenders refinement, not required here).

**Two documented limitations (acceptable for the Weaver threat model):**
1. **Entrypoint transition** — at the instant crun `execve`s the container entrypoint, the process is still moving into the `container` cgroup, so `matchData == "container"` does NOT match the entrypoint exec. Every exec *by a running container process* (descendants) IS matched. Consequence: a container's very first exec escapes; all subsequent execs are fenced. For Weaver agents the entrypoint is a known/allowlisted binary (python3) and the threat is what it execs later — which IS fenced. A malicious *entrypoint* would run once. `perimeter-capstone.sh`'s `echo` test only exercises the entrypoint (echo is a shell builtin → no descendant exec) so it shows a false "not blocked" — update it to `sh -c 'id'` to test the real surface.
2. **Sigkill vs Override** — Sigkill terminates the process synchronously but doesn't guarantee the syscall's effect is prevented; adding an `Override` action would, but `Override` collides with the §4.1 `Sigkill`-only operator verbatim (lint `test_tetragon_policy_verbatim` forbids non-Sigkill actions). Left as Sigkill-only; revisit if guaranteed prevention is required.

Also unresolved earlier note: a container entrypoint that IS an allowlisted binary is fexecve'd as `/dev/fd/N` — moot given the entrypoint-transition gap already lets entrypoints through.
