...
| Mirostat v2 sampling | `crates/sovereign-sampler/` `Mirostat` — stateful perplexity-targeting decode controller (Basu et al.): targets constant surprise τ via a running μ threshold (truncate within μ → sample → nudge μ by observed-vs-target error), holding output perplexity steady regardless of per-step peakedness. Outside the stateless `distribution()` pipeline (μ persists). Commit `3f626c6`; 4 unit (μ init = 2τ, control-law direction, observed surprise converges near τ over 4000 steps, empty-support → None) |
