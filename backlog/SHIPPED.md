...
| Data-structures plane II — roaring bitmap + segment tree + bitops | `run_strategies_demo` runs three previously-**zero-consumer** primitive crates: `sovereign-roaring-bitmap` (`RoaringBitmap::insert`/`intersection` — compact id-set algebra ANDs two posting lists to their 3 shared doc-ids), `sovereign-segment-tree` (`SegmentTree::zeros`/`range_add`/`range_sum`/`range_max` — lazy-propagation range-add + range-query in `O(log n)` for windowed token accounting: two overlapping +adds give sum 15, peak 5), and `sovereign-bitops` (`popcount` = 5, `vpternlog` imm `0xE8` = 3-input majority = `0b1110` — the scalar references for the M008 AVX-512 bit tricks). The set/range/bit substrate retrieval and serving run on. Makes all three actually used by `sovereign-inference-demo`. Commit `507ee58`; prints `data structs II : posting_and=3 win_sum=15 win_max=5 popcount=5 ternlog_maj=0b1110` (demo asserts the line) |
| Generation-integrity plane — retrieval metrics + balance constraint + bounded block | `run_strategies_demo` runs three previously-**zero-consumer** crates: `sovereign-retrieval-metrics` (`precision_at_k`/`reciprocal_rank` — scores a ranking against known-relevant ids: p@3=0.667, MRR=1.0), `sovereign-balanced-constrain` (`BalanceConstraint::is_balanced` — the pushdown automaton a constrained sampler consults to enforce the *context-free* "every `{`/`[` is closed" that a regex cannot: accepts `{"items":[1,2,3]}`, rejects `{"items":[1,2}`), and `sovereign-bounded-block` (`BoundedDecoderBlock::from_weights`/`step` — a StreamingLLM sink+window decoder block that held **10** KV entries over **1000** steps: O(1) memory, what makes endless operation actually endless). Measure retrieval, keep structured output balanced, decode forever in bounded memory. Makes all three actually used by `sovereign-inference-demo`. Commit `3ed9348`; prints `gen integrity : p@3=0.667 mrr=1.000 balanced_ok=true unbalanced=false seen=1000 retained=10 cap=10` (demo asserts the line) |
| Agent-orchestration plane — workflow graph + DAG schedule + task lifecycle | `run_strategies_demo` runs three previously-**zero-consumer** orchestration crates: `sovereign-workflow-graph` (E0552 Plan/Compile — `WorkflowGraph::validate`/`topological_order` validates a DAG of 4 typed nodes read→draft→apply→commit and orders them), `sovereign-dag-schedule` (`Dag::add_dependency`/`waves`/`critical_path_length` — turns "X before Y" constraints into 4 parallel waves with a critical path of 4), and `sovereign-task-lifecycle` (E0548 — `TaskState::can_transition_to` permits Active→Completed but forbids the illegal Active→Archived jump). Map → plan → schedule → run: how an agent turns intent into a runnable, legally-sequenced plan. Makes all three actually used by `sovereign-inference-demo`. Commit `cc6a4e6`; prints `orchestration : wf_ok=true wf_order=4 waves=4 critical=4 legal=true illegal=false` (demo asserts the line) |
| Runtime-governance plane — config resolver + typed state + mode-default policy | `run_strategies_demo` runs three previously-**zero-consumer** governance crates: `sovereign-config-resolver` (E0476 — `LayeredConfig::set`/`resolve` stacks 7 config layers by precedence so the supreme **Policy** layer overrides a **Runtime** `route`=`cloud-gpt` with `local-only`), `sovereign-typed-state` (E0557 Critical Data Flow Law — `TypedState::populated_components` counts how many of the 8 typed components a state carries, not a transcript: 3 here), and `sovereign-mode-default-policy` (`ModeDefaultPolicy::canonical`/`landing_mode` — each trust bundle's boot mode: Private→Plan, Fast→Execute). Resolve config, hold typed state, pick the default execution mode. Makes all three actually used by `sovereign-inference-demo`. Commit `d615484`; prints `governance II : cfg_layer=Policy cfg_val="local-only" state_components=3 private_mode=Some(Plan) fast_mode=Some(Execute)` (demo asserts the line) |
| Conversation-state plane — fork event + search index (over conversation-thread) | `run_strategies_demo` builds a `sovereign-conversation-thread` (3 turns, 2 operator) and runs two previously-**zero-consumer** crates over it: `sovereign-conversation-fork-event` (`ForkLog::record`/`descendants_of` — logs an operator forking `explore-ternary` off turn 1 of `main`, validated against the real thread; 1 descendant of main) and `sovereign-conversation-search-index` (`SearchIndex::add`/`search` — substring + role + branch search finds the 1 turn mentioning `ternary`). The branching multi-turn history an agent works over: fork branches, search across them. Makes both actually used by `sovereign-inference-demo`. Commit `b405487`; prints `conversation : op_turns=2 fork_ok=true descendants=1 search_hits=1` (demo asserts the line) |
| Distributed-runtime plane — CRDT + interval schedule + retry backoff | `run_strategies_demo` runs three previously-**zero-consumer** distributed-runtime crates: `sovereign-crdt` (`GCounter::increment`/`merge`/`value` — two replicas count concurrently offline, 3 and 5, then merge to the same total **8** without coordination), `sovereign-interval-schedule` (`max_weight` — admits the max-weight set of non-overlapping jobs onto one GPU timeline: 2 jobs, weight 7), and `sovereign-retry` (`RetryPolicy::delay_for`/`should_retry` — exponential backoff 100→400ms, retries attempt 3 of 5). Converge shared state, admit non-conflicting jobs, back off on transient failure. Makes all three actually used by `sovereign-inference-demo`. Commit `fbc1974`; prints `distributed rt : crdt_val=8 converged=true admitted=2 admit_weight=7 backoff0=100 backoff2=400 retry3=true` (demo asserts the line) |
| Agent-lifecycle-gates plane — task map + doctrine citation + codegen pipeline | `run_strategies_demo` runs three previously-**zero-consumer** E-series gate crates: `sovereign-task-map` (E0551 Map — `TaskMap::gather`/`missing_components` builds a Code-domain map, 5 of 7 components still missing → incomplete: "MAP prevents blind action"), `sovereign-doctrine-citation` (`cite` — a `ToolInvocation` action in Execute mode carries **4** doctrine tags from `sovereign-doctrinal-preservation`), and `sovereign-codegen-pipeline` (E0216 — `CodegenStep::next` advances Propose→ValidateCaps and `PromotionRung::can_promote_to` allows AdHoc→SandboxedScript but forbids skipping straight to TrustedPrimitive: generated code climbs one rung at a time, never trusted on arrival). Map → cite → gate: the trust gates around every agent action. Makes all three actually used by `sovereign-inference-demo`. Commit `82d87e0`; prints `lifecycle gates : map_missing=5 map_complete=false cite_tags=4 step_next=Some(ValidateCaps) promote_ok=true promote_skip=false` (demo asserts the line) |
| Sovereignty-policy plane — policy input + choice envelope | `run_strategies_demo` runs two previously-**zero-consumer** decision-contract crates: `sovereign-policy-input` (E0474 — the 10-field authorization question with **7** `PolicyQuestion`s; crucially it carries *intent*, so the identical `(agent:scout, read, ~/.ssh/config)` triple resolves differently under "summarize my files" vs "debug ssh failure" — same triple, different intent, different decision) and `sovereign-choice-envelope` (M042 — the 9-axis sovereignty boundary: `empty_canonical` seeds all 9 axes, `set_side`/`side_of` records + reads one axis's chosen side with a reason, and `validate` enforces the canonical invariants). The authorization question + the boundary it resolves against. Makes both actually used by `sovereign-inference-demo`. Commit `2c881b1`; prints `sovereignty pol : questions=7 same_triple=true intent_differs=true axes=9 side0=Some(Left) envelope_ok=true` (demo asserts the line) |
| Decode-loop plane — the assembled autoregressive inner loop (crown jewel) | `run_strategies_demo` runs the previously-**zero-consumer** `sovereign-decode-loop` — the transformer decode inner loop assembled from the three primitive engines: `OutputHead::new` (vocab×dim logit projection), `DecodeLoop::new`(rope+attention+sampler+head), `append_token` (grows the RoPE-rotated KV cache, each key rotated by its own position), and `decode_next` (rotates the query, attends over the cache via `sovereign-attention`, projects context→vocab logits, samples one token via `sovereign-sampler`) — deterministic under a seed, fully replayable. A 3-token cache emits 2 deterministic tokens over a 3-vocab. This is the loop a real autoregressive decoder runs, now exercised end-to-end in the running binary (also pulls in the last two primitive engines `sovereign-rope` + `sovereign-attention` as direct demo deps). Makes it actually used by `sovereign-inference-demo`. Commit `_PENDING_`; prints `decode loop : cache_len=3 tok1=0 tok2=0 emitted=2 vocab=3` (demo asserts the line) |
