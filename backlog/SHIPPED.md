...
| Distributed-runtime plane — CRDT + interval schedule + retry backoff | `run_strategies_demo` runs three previously-**zero-consumer** distributed-runtime crates: `sovereign-crdt` (`GCounter::increment`/`merge`/`value` — two replicas count concurrently offline, 3 and 5, then merge to the same total **8** without coordination), `sovereign-interval-schedule` (`max_weight` — admits the max-weight set of non-overlapping jobs onto one GPU timeline: 2 jobs, weight 7), and `sovereign-retry` (`RetryPolicy::delay_for`/`should_retry` — exponential backoff 100→400ms, retries attempt 3 of 5). Converge shared state, admit non-conflicting jobs, back off on transient failure. Makes all three actually used by `sovereign-inference-demo`. Commit `_PENDING_`; prints `distributed rt : crdt_val=8 converged=true admitted=2 admit_weight=7 backoff0=100 backoff2=400 retry3=true` (demo asserts the line) |
