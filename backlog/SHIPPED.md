...
>
> The catalogue is `backlog/INDEX.md` (84 milestone files / 14,079 distinct R-rows — INDEX is generated by `scripts/backlog/gen-index.py` since 2026-07-17, so its totals are the authoritative live figures). This file is the orthogonal "delivery state" view per the operator's standing constraint:
>
...
| State | R-rows | % of 14,079 |
|---|---:|---:|
| Catalogued (total) | 14,079 | 100% |
...
| Surface | Status |
|---|---|
| `backlog/INDEX.md` (80 milestones, 13,740 R-rows enumerated — point-in-time figure from that session; current totals live in the generated `backlog/INDEX.md`) | shipped prior to this session; surfaces the catalogue at a glance |
...
> **Superseded (recorded 2026-07-17):** that operator decision has since landed — SDD-710 sanctions a single vetted `unsafe` carve-out in `crates/sovereign-simd` (explicit carve-out comment in the crate); the workspace-wide `forbid` stands for everything else. The boundary note above is retained as the point-in-time record.
