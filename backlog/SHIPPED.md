...
| F06046/F06067-F06070, R12107-R12110 — energy monitor | `EnergyReport` + `OpCount::energy_report` + block-level `BitLinearMlp`/`TernarySwiGlu::energy_report` — the dump's add/sub-vs-FP-MUL accounting as a real profile: multiplications eliminated, energy-saving ratio, weight sparsity; composes across a whole FFN | `e1bb13a`, `14999ae` | 5 unit (single-layer accounting, sparsity measurement, block-level MLP + SwiGLU reports, serde) |
