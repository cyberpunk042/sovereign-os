...
| selfdef audit-chain tamper ALERT (cockpit security) | `config/prometheus/alerts/selfdef-audit-chain.rules.yml` — 3 critical alerts (guardian/perimeter/scheduler) firing on `selfdef_<sub>_audit_chain_events == -1` (SHA-256 chain integrity broken: corruption/tampering/concurrent writer). Cockpit-side tamper alert (sovereign-os Prometheus scrapes the selfdef hosts), complementing the host-local rules. 3 runbook sections (preserve-then-investigate) + `test_selfdef_audit_chain_alerts_contract.py` (locks the -1 sentinel) + generic anchor/metric-lockstep gates. |
