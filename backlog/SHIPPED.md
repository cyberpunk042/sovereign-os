...
| D-22 Language Model Status & Operability panel | New cockpit dashboard `webapp/d-22-lm-status-operability/index.html` + read-only API daemon `scripts/operator/lm-status-operability-api.py` (`GET /api/lm-status/{health,devices,stream}`, POST/PUT/DELETE→405) + systemd unit `sovereign-lm-status-operability-api.service` (R171 hardened, loopback :8122). Renders per-device (CPU0/GPU0/GPU1 = Conductor/Logic/Oracle per M075) Model 0/1/2 tabs + History\|Selected latency + operability Action/Test buttons + render-only Chat; reuses the shared `scripts/inference/model-health.py` core (no new data source). Read-only per R10212 — Actions clipboard-copy MS003-signed CLI verbs. Registered in surface-map (`lm-status-operability`, 4/8 at structural ceiling) + nav-snippet DASHBOARDS (D-22). Local sketch panel 2 of 2 (D-21 orchestration follows) | `983ebde` | 14 contract tests in `test_d22_lm_status_operability_webapp_contract.py` (html5 + sovereign meta/footer, zero external deps, same-origin fetches, read-only 405, live daemon /webapp/ 200, devices shape CPU0/GPU0/GPU1 × Model 0/1/2, /version webapp surface, surface-map shipped, nav registered) + surface-map governance reconcile | n/a (sovereign-os-owned cortex per M075 R12530) |
| D-21 Language Model Orchestration panel | New cockpit dashboard `webapp/d-21-lm-orchestration/index.html` + read-only API daemon `scripts/operator/lm-orchestration-api.py` (`GET /api/lm-orchestration/{grid,profiles,features,stream}`, POST/PUT/DELETE→405) + systemd unit `sovereign-lm-orchestration-api.service` (R171 hardened, loopback :8121). Composes three shipped sources (no new data model): model-health snapshot → GPU0/GPU1/Ext-GPU/CPU0 assignment grid (Model 0/1/2 + Mode + CPU core-ranges per M075), runtime-modes `_list_profiles()` → M076 Profiles row, `/proc/cpuinfo` → AVX-512 Features CPU + GPU capability. Read-only per R10212 — Apply clipboard-copies the MS003-signed profile verb. SDD-040 palette + SDD-045 control-surface compliant (both new panels). Registered in surface-map (`lm-orchestration`, 4/8 ceiling) + nav-snippet (D-21) + `config/dashboard-catalog.yaml`. Local sketch panel 1 of 2 | `ddb068b` | 17 contract tests in `test_d21_lm_orchestration_webapp_contract.py` (html5 + meta/footer, zero deps, same-origin, read-only 405, live daemon grid/profiles/features shapes, canonical palette + --mono, control-surface inlined, surface-map shipped, nav registered) + surface-map governance reconcile (24 modules) | n/a (sovereign-os-owned cortex per M075 R12530) |
| D-23 Model Catalog panel | New cockpit dashboard `webapp/d-23-models-catalog/index.html` + read-only API daemon `scripts/operator/models-catalog-api.py` (`GET /api/models-catalog/{catalog,stream}`, POST/PUT/DELETE→405) + systemd unit `sovereign-models-catalog-api.service` (R171 hardened, loopback :8123). Flips the dashboard-catalog `models-catalog` PLANNED surface to LIVE: renders models/catalog.yaml grouped by SRP tier (id/class/engine/precision/params/context/license/purpose/status). Reuses `scripts/inference/model-health.py` load_catalog (no new data source). SDD-040 palette + SDD-045 control-surface compliant. Read-only per R10212 — model lifecycle = signed `sovereign-osctl models …`. Registered in surface-map (`models-catalog`, 4/8 ceiling) + nav-snippet (D-23) + dashboard-catalog. | `62feaec` | 16 contract tests in `test_d23_models_catalog_webapp_contract.py` (html5 + meta/footer, zero deps, same-origin, read-only 405, live daemon /webapp/ 200, catalog tier-grouping shape, /version webapp surface, canonical palette + control-surface, surface-map shipped, nav) + surface-map governance reconcile (25 modules) | n/a (sovereign-os-owned catalog per M075) |
| D-24 CPU Features panel | New cockpit dashboard `webapp/d-24-cpu-features/index.html` + read-only API daemon `scripts/operator/cpu-features-api.py` (`GET /api/cpu-features/{probe,workloads,advisory,stream}`, POST/PUT/DELETE→405) + systemd unit `sovereign-cpu-features-api.service` (R171 hardened, loopback :8124). Flips the dashboard-catalog `cpu-features` PLANNED surface to LIVE: the deep AVX-512 capability view — extension map + per-AI-workload fit + advisory. Reuses `scripts/hardware/avx512-advisor.py` (probe/workloads/advisory --json; no drift). SDD-040 palette + SDD-045 control-surface compliant. Read-only per R10212 (pure capability observation). Registered in surface-map (`cpu-features`, 4/8 ceiling) + nav-snippet (D-24) + dashboard-catalog. | `cd05d45` | 13 contract tests in `test_d24_cpu_features_webapp_contract.py` (html5 + meta, zero deps, same-origin, read-only 405, live daemon /webapp/ 200 + /probe shape, canonical palette + control-surface, surface-map shipped, nav) + governance reconcile (26 modules) | n/a (sovereign-os-owned hardware capability per M008/M074) |
| Orchestration-intent profile family | New `profiles/orchestration/` family (SEPARATE from the §18 runtime family, which stays verbatim-locked at 3): 5 operator-named intent profiles (`full-orchestration`, `coding-focus`, `thinking-focus`, `hybrid-coding-thinking`, `full-hybrid`) + new `schemas/orchestration-profile.schema.yaml` (distinct top-level key `orchestration_profile` — no collision with the runtime lint) + `tests/lint/test_orchestration_profiles.py` + `tests/schema/test_orchestration_profile_schema_conformance.py`. Each profile's `allocations[]` reuse real `models/catalog.yaml` model ids by SRP tier (pulse/logic/oracle). Wires D-21's Profiles row to merge runtime (3) + orchestration (5) = 8, each with an `apply_cmd`. Registered in `test_schemas_consistency.py` (6-schema set). | `c054e3ed` | `test_orchestration_profiles.py` (5-file set, schema shape, id↔filename, engine/model ∈ catalog, top-level key, runtime family untouched at 3) + conformance test (structural: file/id/distinct-key; jsonschema-validate skips when lib absent) + D-21 contract still green (lists 8 profiles) + schema-consistency 6-set | n/a (sovereign-os-owned orchestration per M075) |
| D-25 Self-Defense Management panel | New cockpit dashboard `webapp/d-25-selfdef-management/index.html` + read-only **consumer** API daemon `scripts/operator/selfdef-management-api.py` (`GET /api/selfdef-management/{state,stream}` + `/control-systems`, POST/PUT/DELETE→405) + systemd unit `sovereign-selfdef-management-api.service` (R171 hardened, loopback :8125). Registered as new dashboard-catalog panel `d-25-selfdef-management` (`api: sovereign-selfdef-management-api`) — the READ-ONLY consumer complement to the base `selfdef-management` surface, whose entry is left untouched: unified READ-ONLY view of the selfdef IPS — on/off enablement + M060 mirror-chain health (N/10 fresh + aggregate state) + links to the 6 domain panels D-13..D-18. **Boundary R10212 (load-bearing):** sovereign-os is the CONSUMER, selfdef the PRODUCER — derives state via the sanctioned `scripts/operator/m060-health.py` `probe()` proxy, NEVER mutates selfdef; on/off is the SDD-045 `selfdef` control (`sovereign-osctl selfdef {on\|off}`, clipboard-copy only). Graceful `unreachable` envelope when the producer socket is absent. SDD-040 palette + SDD-045 control-surface (filterSlug `selfdef-management`) compliant. Registered in surface-map (`selfdef-management`, 4/8 ceiling) + nav-snippet (D-25, also fixes D-22 ordering) + dashboard-catalog. | `9ddae5eb` | 15 contract tests in `test_d25_selfdef_management_webapp_contract.py` (html5 + meta/footer, R10212 boundary meta, zero deps, same-origin, read-only 405, live daemon /webapp/ 200 + state consumer-envelope shape, /version webapp surface, canonical palette + control-surface filterSlug, surface-map shipped, nav) + governance reconcile (27 modules; `selfdef-management` added to structural-below-8 set) | selfdef producer (`sovereign-osctl selfdef …`) — consumed READ-ONLY per R10212 |
