...
| Unified composable generation | `crates/sovereign-decoder-stack/` `GenOptions` (builder: max_new · base `LogitMask` · `no_repeat_ngram` · `stop_tokens`) + `generate_with` (one loop composing constrained masking + dynamic no-repeat-ngram + early-stop + per-token streaming, which the single-purpose methods could only do separately) + `crates/sovereign-llm/` `SovereignLlm::generate_ids_with` (same at the text-to-text API). Commits `de0d539`, `fe49190`; 2 stack unit (all controls compose; plain == generate) + 2 llm unit (composes + streams + reproducible; empty-prompt) |
