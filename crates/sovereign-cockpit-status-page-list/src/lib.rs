...
impl StatusPageList {
    ...
    pub fn register(&mut self, page: StatusPage) -> Result<(), StatusError> {
        if page.id.is_empty() {
            return Err(StatusError::EmptyId);
        }
        if page.label.is_empty() {
            return Err(StatusError::EmptyLabel);
        }
        if page.url.is_empty() {
            return Err(StatusError::EmptyUrl);
        }
...
mod tests {
    ...
    fn register_and_list() {
        ...
        l.register(page("github", "GitHub", State::Operational))
            .unwrap();
        l.register(page("aws-us-east-1", "AWS US-EAST-1", State::Operational))
            .unwrap();
    ...
    fn duplicate_rejected() {
        ...
        l.register(page("a", "A", State::Operational)).unwrap();
        assert!(matches!(
            l.register(page("a", "A", State::Operational)).unwrap_err(),
            StatusError::DuplicateId(_)
        ));
    }
    ...
    fn unknown_id() {
        let mut l = StatusPageList::new();
        assert!(matches!(
            l.update_state("nope", State::Operational, 0).unwrap_err(),
            StatusError::UnknownId(_)
        ));
    }
    ...
    fn empty_fields_rejected() {
        ...
        bad2.url = "".into();
        assert!(matches!(
            l.register(bad2).unwrap_err(),
            StatusError::EmptyUrl
        ));
    }
