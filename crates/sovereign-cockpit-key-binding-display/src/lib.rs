...
impl KeyBindingDisplay {
    ...
    pub fn render(&self, c: &Chord) -> String {
        let sep = if self.platform == Platform::Mac {
            ""
        } else {
            "+"
        };
        ...
        match self.platform {
            ...
            _ => {
                ...
                if c.meta {
                    parts.push(if self.platform == Platform::Windows {
                        "Win".into()
                    } else {
                        "Super".into()
                    });
    ...
    pub fn validate(&self) -> Result<(), DisplayError> {
        if self.schema_version != SCHEMA_VERSION {
            return Err(DisplayError::SchemaMismatch);
        }
        Ok(())
    }
}
