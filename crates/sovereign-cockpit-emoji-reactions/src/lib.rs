...
impl EmojiReactions {
    ...
    pub fn toggle(
        ...
        emoji: &str,
        user_id: &str,
    ) -> Result<ToggleVerdict, ReactionError> {
        if message_id.is_empty() {
            return Err(ReactionError::EmptyMessage);
        }
        if emoji.is_empty() {
            return Err(ReactionError::EmptyEmoji);
        }
        if user_id.is_empty() {
            return Err(ReactionError::EmptyUser);
        }
    ...
    pub fn counts(&self, message_id: &str) -> Vec<(String, u64)> {
        let Some(m) = self.messages.get(message_id) else {
            return Vec::new();
        };
    ...
    pub fn validate(&self) -> Result<(), ReactionError> {
        if self.schema_version != SCHEMA_VERSION {
            return Err(ReactionError::SchemaMismatch);
        }
...
mod tests {
    ...
    fn empty_inputs_rejected() {
        let mut r = EmojiReactions::new();
        assert!(matches!(
            r.toggle("", "e", "u").unwrap_err(),
            ReactionError::EmptyMessage
        ));
        assert!(matches!(
            r.toggle("m", "", "u").unwrap_err(),
            ReactionError::EmptyEmoji
        ));
        assert!(matches!(
            r.toggle("m", "e", "").unwrap_err(),
            ReactionError::EmptyUser
        ));
    }
