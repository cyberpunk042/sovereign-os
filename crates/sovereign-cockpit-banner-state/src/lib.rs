...
pub fn compute_severity(
    ...
    if worst_thermal == ThermalVerdict::Shutdown {
        return BannerSeverity::Critical;
    }
    if open_alerts > 5 {
        return BannerSeverity::Critical;
    }
    if worst_thermal == ThermalVerdict::Throttle {
        return BannerSeverity::Warn;
    }
    if open_alerts >= 1 {
        return BannerSeverity::Warn;
    }
    if worst_thermal == ThermalVerdict::Warm {
        return BannerSeverity::Notice;
    }
...
impl BannerState {
    ...
    pub fn validate(&self) -> Result<(), BannerError> {
        ...
        if computed != self.severity {
            return Err(BannerError::SeverityMismatch {
                stored: self.severity,
                computed,
            });
        }
        Ok(())
...
mod tests {
    ...
    fn calm_when_quiet_and_planning() {
        assert_eq!(
            compute_severity(ExecutionMode::Plan, ThermalVerdict::Cool, 0),
            BannerSeverity::Calm
        );
    }
    ...
    fn execute_mode_promotes_to_notice() {
        assert_eq!(
            compute_severity(ExecutionMode::Execute, ThermalVerdict::Cool, 0),
    ...
    fn warm_thermal_is_notice() {
        assert_eq!(
            compute_severity(ExecutionMode::Plan, ThermalVerdict::Warm, 0),
            BannerSeverity::Notice
        );
    }
    ...
    fn throttle_is_warn() {
        assert_eq!(
            compute_severity(ExecutionMode::Plan, ThermalVerdict::Throttle, 0),
            BannerSeverity::Warn
        );
    }
    ...
    fn alerts_one_to_five_is_warn() {
        assert_eq!(
            compute_severity(ExecutionMode::Plan, ThermalVerdict::Cool, 1),
            BannerSeverity::Warn
        );
        assert_eq!(
            compute_severity(ExecutionMode::Plan, ThermalVerdict::Cool, 5),
            BannerSeverity::Warn
        );
    }
    ...
    fn alerts_over_five_is_critical() {
        assert_eq!(
            compute_severity(ExecutionMode::Plan, ThermalVerdict::Cool, 6),
            BannerSeverity::Critical
        );
        assert_eq!(
            compute_severity(ExecutionMode::Plan, ThermalVerdict::Cool, 100),
            BannerSeverity::Critical
        );
    }
    ...
    fn shutdown_thermal_is_critical_even_with_no_alerts() {
        assert_eq!(
            compute_severity(ExecutionMode::Plan, ThermalVerdict::Shutdown, 0),
            BannerSeverity::Critical
        );
    }
    ...
    fn shutdown_outranks_warn_alerts() {
        assert_eq!(
            compute_severity(ExecutionMode::Plan, ThermalVerdict::Shutdown, 1),
            BannerSeverity::Critical
        );
    }
    ...
    fn severity_serde_kebab() {
        assert_eq!(
            serde_json::to_string(&BannerSeverity::Calm).unwrap(),
            "\"calm\""
        );
        assert_eq!(
            serde_json::to_string(&BannerSeverity::Notice).unwrap(),
            "\"notice\""
        );
        assert_eq!(
            serde_json::to_string(&BannerSeverity::Warn).unwrap(),
            "\"warn\""
        );
        assert_eq!(
            serde_json::to_string(&BannerSeverity::Critical).unwrap(),
            "\"critical\""
        );
    }
