...
impl TrashBuffer {
    ...
    pub fn soft_delete(&mut self, id: &str, payload: &str, now_ms: u64) -> Result<(), TrashError> {
        if id.is_empty() {
            return Err(TrashError::EmptyId);
        }
        if payload.is_empty() {
            return Err(TrashError::EmptyPayload);
        }
    ...
    pub fn validate(&self) -> Result<(), TrashError> {
        if self.schema_version != SCHEMA_VERSION {
            return Err(TrashError::SchemaMismatch);
        }
        if self.ttl_ms == 0 {
            return Err(TrashError::ZeroTtl);
        }
...
mod tests {
    ...
    fn duplicate_rejected() {
        ...
        t.soft_delete("a", "x", 0).unwrap();
        assert!(matches!(
            t.soft_delete("a", "y", 100).unwrap_err(),
            TrashError::DuplicateId(_)
        ));
    }
    ...
    fn empty_inputs_rejected() {
        let mut t = TrashBuffer::new(1000).unwrap();
        assert!(matches!(
            t.soft_delete("", "x", 0).unwrap_err(),
            TrashError::EmptyId
        ));
        assert!(matches!(
            t.soft_delete("i", "", 0).unwrap_err(),
            TrashError::EmptyPayload
        ));
        assert!(matches!(
            TrashBuffer::new(0).unwrap_err(),
            TrashError::ZeroTtl
        ));
    }
