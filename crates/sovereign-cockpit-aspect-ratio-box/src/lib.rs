...
pub fn fit(outer_w: u32, outer_h: u32, w_num: u32, w_den: u32) -> Result<Box2D, AspectError> {
    if outer_w == 0 || outer_h == 0 { return Err(AspectError::ZeroOuter); }
    if w_num == 0 || w_den == 0 { return Err(AspectError::ZeroRatio); }
...
pub fn validate_schema_version(s: &str) -> Result<(), AspectError> {
    if s != SCHEMA_VERSION { return Err(AspectError::SchemaMismatch); }
    Ok(())
}
...
mod tests {
    ...
    fn matches_outer_when_ratio_equal() {
        ...
        assert_eq!(b, Box2D { x: 0, y: 0, w: 200, h: 100 });
    }
    ...
    fn pillarbox_when_outer_wider() {
        ...
        assert_eq!(b, Box2D { x: 150, y: 0, w: 100, h: 100 });
    }
    ...
    fn letterbox_when_outer_taller() {
        ...
        assert_eq!(b, Box2D { x: 0, y: 150, w: 100, h: 100 });
    }
    ...
    fn zero_outer_rejected() {
        assert!(matches!(fit(0, 10, 1, 1).unwrap_err(), AspectError::ZeroOuter));
        assert!(matches!(fit(10, 0, 1, 1).unwrap_err(), AspectError::ZeroOuter));
    }

    #[test]
    fn zero_ratio_rejected() {
        assert!(matches!(fit(10, 10, 0, 1).unwrap_err(), AspectError::ZeroRatio));
        assert!(matches!(fit(10, 10, 1, 0).unwrap_err(), AspectError::ZeroRatio));
    }
