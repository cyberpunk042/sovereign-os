...
impl ColumnVisibility {
    ...
    pub fn register(&mut self, id: &str, label: &str, required: bool) -> Result<(), ColError> {
        if id.is_empty() { return Err(ColError::EmptyId); }
        if label.is_empty() { return Err(ColError::EmptyLabel); }
        if self.columns.iter().any(|c| c.id == id) {
            return Err(ColError::DuplicateId(id.into()));
        }
        self.columns.push(Column { id: id.into(), label: label.into(), required, visible: true });
        Ok(())
    }

    /// Show.
    pub fn show(&mut self, id: &str) -> Result<(), ColError> {
        let c = self.columns.iter_mut().find(|c| c.id == id).ok_or_else(|| ColError::UnknownId(id.into()))?;
        c.visible = true;
        Ok(())
    }
    ...
    pub fn toggle(&mut self, id: &str) -> Result<bool, ColError> {
        let c = self.columns.iter_mut().find(|c| c.id == id).ok_or_else(|| ColError::UnknownId(id.into()))?;
        if c.visible {
            if c.required { return Err(ColError::RequiredCannotHide(id.into())); }
            c.visible = false;
        } else {
            c.visible = true;
        }
    ...
    pub fn visible_columns(&self) -> Vec<&str> {
        self.columns.iter().filter(|c| c.visible).map(|c| c.id.as_str()).collect()
    }

    /// Validate.
    pub fn validate(&self) -> Result<(), ColError> {
        if self.schema_version != SCHEMA_VERSION { return Err(ColError::SchemaMismatch); }
        for c in &self.columns {
            if c.id.is_empty() { return Err(ColError::EmptyId); }
            if c.label.is_empty() { return Err(ColError::EmptyLabel); }
...
mod tests {
    ...
    fn required_cannot_hide() {
        ...
        assert!(matches!(v.hide("a").unwrap_err(), ColError::RequiredCannotHide(_)));
    }
    ...
    fn toggle_required_visible_rejected() {
        ...
        assert!(matches!(v.toggle("a").unwrap_err(), ColError::RequiredCannotHide(_)));
    }

    #[test]
    fn unknown_id_rejected() {
        let mut v = ColumnVisibility::new();
        assert!(matches!(v.hide("nope").unwrap_err(), ColError::UnknownId(_)));
    }

    #[test]
    fn empty_inputs_rejected() {
        let mut v = ColumnVisibility::new();
        assert!(matches!(v.register("", "A", false).unwrap_err(), ColError::EmptyId));
        assert!(matches!(v.register("a", "", false).unwrap_err(), ColError::EmptyLabel));
    }
    ...
    fn duplicate_rejected() {
        let mut v = ColumnVisibility::new();
        v.register("a", "A", false).unwrap();
        assert!(matches!(v.register("a", "B", false).unwrap_err(), ColError::DuplicateId(_)));
    }
    ...
    fn schema_drift_rejected() {
        ...
        assert!(matches!(v.validate().unwrap_err(), ColError::SchemaMismatch));
    }
