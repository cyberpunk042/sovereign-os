...
impl SkeletonTemplate {
    ...
    pub fn register(&mut self, template_id: &str, blocks: Vec<Block>) -> Result<(), SkeletonError> {
        if template_id.is_empty() {
            return Err(SkeletonError::EmptyId);
        }
        if blocks.is_empty() {
            return Err(SkeletonError::EmptyBlocks(template_id.into()));
        }
        for b in &blocks {
            if b.w_px == 0 || b.h_px == 0 {
                return Err(SkeletonError::ZeroDim);
            }
        }
    ...
    pub fn render(
        ...
        template_id: &str,
        count: usize,
    ) -> Result<Vec<RenderedRow>, SkeletonError> {
        let blocks = self
            .templates
            .get(template_id)
    ...
    pub fn validate(&self) -> Result<(), SkeletonError> {
        ...
        for (id, blocks) in &self.templates {
            ...
            if blocks.is_empty() {
                return Err(SkeletonError::EmptyBlocks(id.clone()));
            }
            for b in blocks {
                if b.w_px == 0 || b.h_px == 0 {
                    return Err(SkeletonError::ZeroDim);
...
mod tests {
    ...
    fn circle(d: u32) -> Block {
        Block {
            kind: BlockKind::Circle,
            w_px: d,
            h_px: d,
        }
    }
    ...
    fn register_and_render() {
        ...
        s.register("avatar-name", vec![circle(40), line(200)])
            .unwrap();
    ...
    fn render_unknown() {
        ...
        assert!(matches!(
            s.render("nope", 2).unwrap_err(),
            SkeletonError::UnknownId(_)
        ));
    }
    ...
    fn empty_blocks_rejected() {
        let mut s = SkeletonTemplate::new();
        assert!(matches!(
            s.register("x", vec![]).unwrap_err(),
            SkeletonError::EmptyBlocks(_)
        ));
    }
    ...
    fn empty_id_rejected() {
        let mut s = SkeletonTemplate::new();
        assert!(matches!(
            s.register("", vec![line(10)]).unwrap_err(),
            SkeletonError::EmptyId
        ));
    }
