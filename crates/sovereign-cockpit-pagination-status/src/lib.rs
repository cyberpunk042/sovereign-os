...
impl PaginationStatusFormatter {
    ...
    pub fn format(state: PageState) -> Result<PaginationStatus, StatusError> {
        if state.page == 0 { return Err(StatusError::PageZero); }
        if state.page_size == 0 { return Err(StatusError::PageSizeZero); }
        ...
        let display = if let Some(pre) = pre {
            format!("Showing {}-{} of {} (filtered from {})",
                group_thousands(first), group_thousands(last), total_str, group_thousands(pre))
        } else {
            format!("Showing {}-{} of {}",
                group_thousands(first), group_thousands(last), total_str)
        };
        Ok(PaginationStatus {
            schema_version: SCHEMA_VERSION.into(),
            display,
...
mod tests {
    ...
    fn no_items_message() {
        let s = PaginationStatusFormatter::format(PageState {
            page: 1, page_size: 50, total_items: 0, pre_filter_total: 0,
        }).unwrap();
    ...
    fn first_page_unfiltered() {
        let s = PaginationStatusFormatter::format(PageState {
            page: 1, page_size: 50, total_items: 1000, pre_filter_total: 0,
        }).unwrap();
    ...
    fn last_partial_page() {
        let s = PaginationStatusFormatter::format(PageState {
            page: 21, page_size: 50, total_items: 1005, pre_filter_total: 0,
        }).unwrap();
    ...
    fn filtered_shows_both() {
        let s = PaginationStatusFormatter::format(PageState {
            page: 1, page_size: 50, total_items: 1000, pre_filter_total: 10000,
        }).unwrap();
    ...
    fn pre_filter_equal_to_total_omitted() {
        let s = PaginationStatusFormatter::format(PageState {
            page: 1, page_size: 50, total_items: 1000, pre_filter_total: 1000,
        }).unwrap();
    ...
    fn page_zero_rejected() {
        assert!(matches!(
            PaginationStatusFormatter::format(PageState { page: 0, page_size: 50, total_items: 100, pre_filter_total: 0 }).unwrap_err(),
    ...
    fn page_size_zero_rejected() {
        assert!(matches!(
            PaginationStatusFormatter::format(PageState { page: 1, page_size: 0, total_items: 100, pre_filter_total: 0 }).unwrap_err(),
    ...
    fn bad_pre_filter_rejected() {
        assert!(matches!(
            PaginationStatusFormatter::format(PageState { page: 1, page_size: 50, total_items: 1000, pre_filter_total: 500 }).unwrap_err(),
    ...
    fn schema_drift_rejected() {
        let mut s = PaginationStatusFormatter::format(PageState {
            page: 1, page_size: 50, total_items: 100, pre_filter_total: 0,
        }).unwrap();
    ...
    fn status_serde_roundtrip() {
        let s = PaginationStatusFormatter::format(PageState {
            page: 5, page_size: 50, total_items: 1000, pre_filter_total: 5000,
        }).unwrap();
