...
impl CountdownTimer {
    ...
    pub fn new(duration_ms: u64) -> Result<Self, TimerError> {
        if duration_ms == 0 { return Err(TimerError::ZeroDuration); }
        Ok(Self {
    ...
    pub fn start(&mut self, now_ms: u64) -> Result<(), TimerError> {
        match self.phase {
            Phase::Idle => {
                self.phase = Phase::Running { started_at_ms: now_ms, accumulated_ms: 0 };
                Ok(())
            }
    ...
    pub fn pause(&mut self, now_ms: u64) -> Result<(), TimerError> {
        match self.phase {
            Phase::Running { started_at_ms, accumulated_ms } => {
                let run_elapsed = now_ms.saturating_sub(started_at_ms);
                self.phase = Phase::Paused { accumulated_ms: accumulated_ms.saturating_add(run_elapsed) };
                Ok(())
            }
            _ => Err(TimerError::InvalidTransition),
        }
    ...
    pub fn resume(&mut self, now_ms: u64) -> Result<(), TimerError> {
        match self.phase {
            Phase::Paused { accumulated_ms } => {
                self.phase = Phase::Running { started_at_ms: now_ms, accumulated_ms };
                Ok(())
            }
            _ => Err(TimerError::InvalidTransition),
        }
    ...
    pub fn validate(&self) -> Result<(), TimerError> {
        if self.schema_version_marker != 1 { return Err(TimerError::SchemaMismatch); }
        if self.duration_ms == 0 { return Err(TimerError::ZeroDuration); }
        Ok(())
    }
}
...
mod tests {
    ...
    fn invalid_transitions_rejected() {
        let mut t = CountdownTimer::new(1000).unwrap();
        assert!(matches!(t.pause(0).unwrap_err(), TimerError::InvalidTransition));
        assert!(matches!(t.resume(0).unwrap_err(), TimerError::InvalidTransition));
        t.start(0).unwrap();
        assert!(matches!(t.start(0).unwrap_err(), TimerError::InvalidTransition));
    }

    #[test]
    fn zero_duration_rejected() {
        assert!(matches!(CountdownTimer::new(0).unwrap_err(), TimerError::ZeroDuration));
    }
    ...
    fn schema_drift_rejected() {
        ...
        assert!(matches!(t.validate().unwrap_err(), TimerError::SchemaMismatch));
    }
