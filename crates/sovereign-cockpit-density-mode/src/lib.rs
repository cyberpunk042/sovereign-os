...
impl DensityMode {
    ...
    pub const ALL: [DensityMode; 4] = [
        DensityMode::Compact, DensityMode::Comfortable, DensityMode::Spacious, DensityMode::Touch,
    ];

    /// Canonical spacing for this mode.
    pub fn spacing(self) -> Spacing {
        match self {
            DensityMode::Compact     => Spacing { row_height_px: 28, padding_px:  4, font_pt: 11 },
            DensityMode::Comfortable => Spacing { row_height_px: 36, padding_px:  8, font_pt: 13 },
            DensityMode::Spacious    => Spacing { row_height_px: 48, padding_px: 12, font_pt: 14 },
            DensityMode::Touch       => Spacing { row_height_px: 56, padding_px: 16, font_pt: 16 },
        }
    }
}
...
impl DensityState {
    ...
    pub fn default_state() -> Self {
        Self { schema_version: SCHEMA_VERSION.into(), mode: DensityMode::Comfortable }
    }
    ...
    /// Resolved spacing.
    pub fn spacing(&self) -> Spacing { self.mode.spacing() }
...
mod tests {
    ...
    fn spacing_progression_monotonic_row_height() {
        let order = [DensityMode::Compact, DensityMode::Comfortable, DensityMode::Spacious, DensityMode::Touch];
    ...
    fn schema_drift_rejected() {
        ...
        assert!(matches!(s.validate().unwrap_err(), DensityError::SchemaMismatch));
    }

    #[test]
    fn mode_serde_kebab() {
        assert_eq!(serde_json::to_string(&DensityMode::Compact).unwrap(), "\"compact\"");
        assert_eq!(serde_json::to_string(&DensityMode::Comfortable).unwrap(), "\"comfortable\"");
        assert_eq!(serde_json::to_string(&DensityMode::Spacious).unwrap(), "\"spacious\"");
        assert_eq!(serde_json::to_string(&DensityMode::Touch).unwrap(), "\"touch\"");
    }
