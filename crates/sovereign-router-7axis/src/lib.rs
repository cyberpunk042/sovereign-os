...
//! - all in-between → Logic Engine (RTX 5090 NVFP4, per D-022)
//!
...
pub enum SrpRole {
    ...
    Conductor,
    /// Logic Engine — RTX 5090 NVFP4 (D-022). Mid cost.
    Logic,
...
fn decide_role(a: &TaskAxes) -> SrpRole {
    ...
    // 3. GUI domain → Logic Engine (RTX 5090 browser/CUDA paths).
    // Must precede the simple+cheap+fast Conductor path: GUI inherently
    // needs the 5090 substrate even for "simple" requests.
...
fn explain(a: &TaskAxes, role: SrpRole) -> String {
    match role {
        ...
        SrpRole::Logic => format!(
            "Logic Engine (5090 NVFP4): {:?} domain or complex+careful",
            a.domain
        ),
        SrpRole::Oracle => {
            if a.privacy == Privacy::Private && a.safety == Safety::Risky {
