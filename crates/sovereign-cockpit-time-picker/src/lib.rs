...
impl TimePicker {
    ...
    pub fn new(
        ...
        if hour >= 24 {
            return Err(TimePickerError::BadHour(hour));
        }
        if minute >= 60 {
            return Err(TimePickerError::BadMinute(minute));
        }
    ...
    pub fn validate(&self) -> Result<(), TimePickerError> {
        ...
        if self.hour >= 24 {
            return Err(TimePickerError::BadHour(self.hour));
        }
        if self.minute >= 60 {
            return Err(TimePickerError::BadMinute(self.minute));
        }
...
mod tests {
    ...
    fn new_bad_hour_rejected() {
        assert!(matches!(
            TimePicker::new(24, 0, 1, DisplayStyle::H24).unwrap_err(),
            TimePickerError::BadHour(24)
        ));
    }
    ...
    fn new_bad_minute_rejected() {
        assert!(matches!(
            TimePicker::new(0, 60, 1, DisplayStyle::H24).unwrap_err(),
            TimePickerError::BadMinute(60)
        ));
    }
    ...
    fn new_bad_step_rejected() {
        assert!(matches!(
            TimePicker::new(0, 0, 7, DisplayStyle::H24).unwrap_err(),
            TimePickerError::BadStep(7)
        ));
    }
    ...
    fn display_h12_am_pm() {
        assert_eq!(
            TimePicker::new(0, 0, 5, DisplayStyle::H12)
                .unwrap()
                .display(),
            "12:00 AM"
        );
        assert_eq!(
            TimePicker::new(12, 30, 5, DisplayStyle::H12)
                .unwrap()
                .display(),
            "12:30 PM"
        );
        assert_eq!(
            TimePicker::new(13, 45, 5, DisplayStyle::H12)
                .unwrap()
                .display(),
            "1:45 PM"
        );
        assert_eq!(
            TimePicker::new(9, 5, 5, DisplayStyle::H12)
                .unwrap()
                .display(),
            "9:05 AM"
        );
    ...
    fn schema_drift_rejected() {
        ...
        assert!(matches!(
            ...
            TimePickerError::SchemaMismatch
        ));
    }
    ...
    fn style_serde_kebab() {
        assert_eq!(
            serde_json::to_string(&DisplayStyle::H12).unwrap(),
            "\"h12\""
        );
        assert_eq!(
            serde_json::to_string(&DisplayStyle::H24).unwrap(),
            "\"h24\""
        );
    }
