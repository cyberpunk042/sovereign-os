...
#
# Operator-named (§1b mandate row): "GPU too, watts, RTX 4090 details
...
" || fail "card filter"
pass "6. --card 'RTX 4090' filter narrows to 4 entries"
...
# RTX 4090 typical = 220 W; PRO 6000 typical = 380 W
assert d['rtx_4090']['watts'] == 220
...
out_oc="$(python3 "${SCRIPT}" budget --mode-4090 oc-peak --mode-pro6000 oc-peak --json)"
...
err="$(python3 "${SCRIPT}" show 'RTX 4090' no-such-mode --json 2>&1 1>/dev/null)" || true
echo "${err}" | python3 -c "
import json, sys
d = json.loads(sys.stdin.read())
