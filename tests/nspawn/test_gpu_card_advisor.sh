...
# RTX 4090 + RTX PRO 6000 operator-specific advisory.
...
grep -q "R271\|E1.M13" "${SCRIPT}" && ok "script cites R271/E1.M13" \
  || ko "R271 missing"
grep -q "RTX 4090" "${SCRIPT}" && ok "KNOWN_CARDS contains RTX 4090" \
  || ko "RTX 4090 missing"
...
d = json.load(sys.stdin)
assert d['round'] == 'R271', d
for f in ('rtx_4090_present','rtx_pro_6000_present','sain01_dual_card_layout','findings'):
    assert f in d, f'missing {f}'
assert isinstance(d['rtx_4090_present'], bool)
...
# RTX 4090 must cite the operator-stated 'slightly reduced' quote.
adv_4090 = ' '.join(m.KNOWN_CARDS['RTX 4090']['advisories'])
assert 'slightly reduced' in adv_4090, adv_4090
assert '280' in adv_4090 and '320' in adv_4090, adv_4090
...
" \
  && ok "KNOWN_CARDS contract: 4090 cites 'slightly reduced'/280-320W, PRO 6000 cites Blackwell/96GB/driver-565/PCIe5" \
...
m = importlib.util.module_from_spec(spec)
spec.loader.exec_module(m)
c = m.classify_card('NVIDIA GeForce RTX 4090')
...
" \
  && ok "operator_recommended_cap_watts band for RTX 4090 = [280, 320]" \
  || ko "cap band wrong"

# ---- human render: banner + RTX 4090 mentioned in dual-card output (when absent, the no-detection path emits message) ----
...
echo "${out_h}" | grep -q "RTX 4090:" \
  && ok "dual-card human shows 4090 detection status" || ko "4090 status line missing"
