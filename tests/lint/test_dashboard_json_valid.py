...
def test_dashboard_uses_sovereign_os_metrics(dash: pathlib.Path):
    ...
    sovereign-os consumes, selfdef enforces), OR sovereign_telemetry_* (the
    dedicated `sovereign-telemetry` probe binary's namespace, M045/M013)."""
    text = dash.read_text()
    assert (
        ...
        f"sovereign_telemetry_* metric — is this dashboard tagged correctly?"
    )
